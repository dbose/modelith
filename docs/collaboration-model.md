...
> git merges), sharded generation state (§6.2), the debt valve
> (`mdl unmanage --reason --expires`, `.mdl/debt.yaml`, drift escalation, §7),
> `alignment_status: proposed/accepted` + `mdl ontology promote` (§5.1),
> `mdl init --workspace` (topology, CODEOWNERS, .code-workspace, §2.1), and the
> per-persona surfaces (LSP for engineers/architects, canvas, CLI, drift bot
> CI templates). Known divergence: `.mdl/state` lives beside the generated dbt
> files (it is their merge base) and `decisions/lock/debt` live under the model
> dir — not at repo root as §2.1 sketches; path configurability via
> mdl-project.yaml is the planned reconciliation. The SME web app (§5.1) and
> GitHub branch-protection settings (§6.3) are platform-side and not shipped
> in-repo. Naming: `model/` singular is scaffolded, per this document's own
> naming caution.
