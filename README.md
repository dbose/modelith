...
| **SME app** | `mdl glossary` — a git-native glossary app (second Vite entry at /sme): SMEs browse a subject-area-scoped term list, see definitions/synonyms/steward/**where-used**, edit only the narrow surface (definition/synonyms/steward/alignment-proposal), and **Submit for review** opens a `sme/<user>/<slug>` PR with a `Co-authored-by` trailer — no git, no CLI, no ERD | ✅ done, verified in-browser |
