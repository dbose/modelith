...
```
model/
  mdl-project.yaml           # project config, dbt target, platform targets, ontology stack
  conceptual/
    subject-areas/*.yaml
    entities/*.yaml
    terms/*.yaml             # glossary
...
.mdl/
  state/                     # generation snapshots, fingerprints
...
- `models/**/*.sql` with protected regions. For a pattern-bearing entity, emit **working transformation logic**, not a stub. An `scd2` dim emits the full incremental merge; a hub emits the hash-key insert. An empty `select 1` is a failed emitter.
